...
> **Declared theoretical RTP: 96.00%** (eligibility window 93–98%) — for **every legal paint**, by construction.

## 1. The wheel

- `N` segments, `N ∈ {8, 10, 12, 14, 16}` (even). The VRF word is mapped to a segment with **rejection sampling**: `limit = floor(256/N)·N`; the first byte `b < limit` gives `segment = b % N`. This is **exactly uniform** — no modulo bias (RANDOMNESS_DICE.md rules). If all 32 bytes were rejected (p < 2⁻³²⁰, never observed), the fallback is `uint256(randomness) % N`, mirrored identically in client and contract.
...
1. every tier ≤ 2
2. `cRisky ≥ 1` — there is something to win
3. `cSafe + cMid ≥ 1` — there is a cushion
4. `2·cRisky ≤ N` — risky covers at most half the wheel
